...
documentation, which explains how to run custom Docker images for Truss
deployments. While following the documented workflow, I identified several areas
where clarity, completeness, and developer usability could be improved,
particularly for users new to Baseten or containerized deployment.

## Workflow tested

I followed the official guide for setting up and deploying a custom base Docker
image, which involved:

- Configuring the `config.yaml` file.
- Building and pushing Docker images.
- Deploying to Baseten.

However, the workflow was difficult to follow and lacked sufficient context. For
example, the documentation states:

> You can build a new base image using Truss's base images as a foundation.

It links to
[Docker Hub](https://hub.docker.com/r/baseten/truss-server-base/tags), where the
latest image was updated three months ago — leaving ambiguity about whether
these images are current, supported, or version-locked.

Additionally, to my knowledge, Baseten doesn't support bringing your own full
`Dockerfile`. Instead, the system generates one from templates to ensure
compatibility with Truss infrastructure. For custom build steps, developers can
use the `build_commands` parameter in `config.yaml`. This distinction should be
clearly documented, ideally surfaced through contextual hints.

While this document claims to be a guide; it is much disticntly not so. Instead
it is a loose set of reference configurations.
...
knowledge, isn't reference in places where it would make sense.
...
alternative (`CustomImage` class).

**Why it matters:** Different developer personas (YAML vs. Chains developers)
benefit from different approaches. Lack of choice forces context switching.
...
I would want to keep users on this guide as they came here to learn how to deploy a custom docker image. 
Linking out to NVIDIA will drop user retetion. 

### Platform-specific build requirements

**Issue:** The guide omits critical platform-specific build instructions —
notably `--platform linux/amd64` for Apple Silicon users.
...
Truss CLI for the `build` command should be spelt out. There seems to be a lot
