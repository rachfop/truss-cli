...
- **You want to use a different base OS** or distribution.
- **You have preconfigured environments** from other projects.
...
1. **Debian-based OS** — The image must be Debian or Debian-like.
2. **Python 3.8 - 3.13** — The image must have a compatible Python version installed.
3. **Absolute Python path** — If the Python executable isn't in the default location, specify its absolute path.
...
Custom Docker images in Truss let you set up your own runtime environment for advanced ML use cases.
Use them to:

- Extend base images with additional dependencies.
- Match existing infrastructure setups.
- Integrate with private registries or CI/CD pipelines.
